...
if (-not $content.Contains("import 'dart:math'")) {
    $content = $content.Replace("import 'package:flutter/material.dart';", "import 'package:flutter/material.dart';`nimport 'dart:math' as math;")
}

$hero_start = $content.IndexOf("  // --- 1. HERO SECTION ---")
$about_start = $content.IndexOf("  // --- 2. ABOUT & EDUCATION ---")

if ($hero_start -ne -1 -and $about_start -ne -1) {
    $new_hero = @"
  // --- 1. HERO SECTION ---
...
"@
    $content = $content.Substring(0, $hero_start) + $new_hero + $content.Substring($about_start)
}
...
if (-not $content.Contains("class AnimatedHeroCircle")) {
